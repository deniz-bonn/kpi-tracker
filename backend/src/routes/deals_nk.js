...
// Berechne gewonnen_datum und gewonnen_monat basierend auf Status
function resolveGewonnenFelder(body, existing = null) {
  const status = body.status;
  const prevStatus = existing?.status;

  // Bereits gewonnen und Datum gesetzt → behalten (außer explizit überschrieben)
  if (status === 'Gewonnen') {
    const gd = body.gewonnen_datum || existing?.gewonnen_datum || body.datum || new Date().toISOString().slice(0, 10);
    const gm = gd.slice(0, 7);
    return { gewonnen_datum: gd, gewonnen_monat: gm };
  }
  // Status weg von Gewonnen → zurücksetzen
...
router.post('/', wrap(async (req, res) => {
  ...
  if (db.dialect === 'postgres') {
    ...
    const row = await db.get(`INSERT INTO deals_nk (${fields.join(',')}) VALUES (${ph}) RETURNING *`, values);
...
router.put('/:id', wrap(async (req, res) => {
  const existing = db.get('SELECT * FROM deals_nk WHERE id=?', [req.params.id]);
