...
export default function Settings() {
  ...
  const upsertRate = useMutation({
    mutationFn: ({ monat, rate }) => exchangeRatesApi.upsert(monat, rate),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['exchange-rates'] }); setRateForm(f => ({ ...f, rate: '' })); },
  });
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      {/* ── TAB: Wechselkurs ───────────────────────────────────────── */}
      {tab === 'exchange' && isAdmin && (
        <section className="rounded-xl border border-gray-200 bg-white p-5">
          <h2 className="text-sm font-semibold text-gray-700 mb-1">Wechselkurs CHF → EUR</h2>
          <p className="text-xs text-gray-500 mb-4">Kurs = Euro pro 1 CHF, gültig pro Monat. Wird für Risem-Beträge (CHF) in Anzeige/Aggregation und bei der AE-Buchung verwendet. Spätere Änderungen wirken nur auf den jeweiligen Monat.</p>
          <form onSubmit={e => { e.preventDefault(); if (rateForm.rate) upsertRate.mutate({ monat: rateForm.monat, rate: Number(rateForm.rate) }); }} className="flex gap-3 mb-4 items-end flex-wrap">
            <div>
              <label className="text-xs text-gray-600 mb-1 block">Monat</label>
              <input type="month" required value={rateForm.monat} onChange={e => setRateForm(f => ({ ...f, monat: e.target.value }))} className={inp} />
            </div>
            <div>
              <label className="text-xs text-gray-600 mb-1 block">1 CHF = … €</label>
              <input type="number" step="0.0001" min="0" required placeholder="1.08" value={rateForm.rate} onChange={e => setRateForm(f => ({ ...f, rate: e.target.value }))} className={inp} />
            </div>
            <button type="submit" disabled={upsertRate.isPending} className="px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded">Speichern</button>
          </form>
          <div className="space-y-1">
            {exchangeRates.length === 0
              ? <p className="text-xs text-gray-400">Noch keine Kurse hinterlegt.</p>
              : exchangeRates.map(r => (
                  <div key={r.monat} className="flex items-center justify-between bg-gray-50 rounded px-3 py-2 text-sm">
                    <span className="text-gray-700">{r.monat}</span>
                    <span className="font-medium text-gray-900">1 CHF = {Number(r.rate).toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 4 })} €</span>
                  </div>
                ))}
          </div>
        </section>
